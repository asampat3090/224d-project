...
class GRUGenerator:
  """ 
  A multimodal gated recurrent unit (GRU) generator
  """
  ...
  def forward(Xi, Xs, model, params, **kwargs):
    ...
    Hin = np.zeros((n, WGRU.shape[0])) # xt, ht-1, bias
    Hout = np.zeros((n, d))
    IFOG = np.zeros((n, d * 4))
  ...
  def backward(dY, cache):
    ...
    for t in reversed(xrange(n)):
      ...
      dWGRU += np.outer(Hin[t], dIFOG[t])
      dHin[t] = dIFOG[t].dot(WGRU.transpose())
    ...
    return { 'WGRU': dWGRU, 'Wd': dWd, 'bd': dbd, 'dXi': dX[0,:], 'dXs': dX[1:,:] }
  ...
  def predict(Xi, model, Ws, params, **kwargs):
    ...
    def LSTMtick(x, h_prev, c_prev):
      ...
      Hin = np.zeros((1,WGRU.shape[0])) # xt, ht-1, bias
      Hin[t,0] = 1
      Hin[t,1:1+d] = x
      Hin[t,1+d:] = h_prev
